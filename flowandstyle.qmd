...
-   never start with numbers; if numbers use leading zeros for alphanumeric sort

-   Dates should always match the yyyy-mm-dd format. For example, 2023-08-01.

## Code Style

-   We adopt a code style that generally matches [tidyverse](https://style.tidyverse.org/index.html) (offer kdv lintr as a resource, recommend settings in RStudio), with some exceptions:

    -   camelCase is always used for variable names. For example, using nestType instead of nest_type.
...
-   508 compliance is always recommend to clients and we always do as much as feasible within the scope of our budget.
...
-   At mimum, KDV always devlopes maps that use colorblind friendly palettes (see [viridis](https://cran.r-project.org/web/packages/viridis/vignettes/intro-to-viridis.html) and [Rcolorbrewer](https://www.datanovia.com/en/blog/the-a-z-of-rcolorbrewer-palette/) packages) and include alternative text (here are some [guidelines](https://supercooldesign.co.uk/blog/how-to-write-good-alt-text) for writing alternative text). We also use headers for organization instead of styling text (define css classes for special styles).
...
-   For internal collaborators within KDV, all work is done within the same repository within the organizations' GitHub account. For external collaborators (i.e., contractors), a fork is created and development occurrs on their own version.

-   Resouces for Git and GitHub:
